...
union task_union init_task_union __attribute__((__section__(".data.init_task")))
   = {{ .state = 0, .priority = 10, .counter = 10, .flags = PF_KTHREAD, .pid = 0, .preempt_count = 0, .need_resched = 0, .next_task = (struct task_struct *)&init_task_union.task, .prev_task = (struct task_struct *)&init_task_union.task, }};


struct task_struct *g_task[128] = {&init_task_union.task,};
...
static struct pt_regs *task_pt_regs(struct task_struct *tsk)
{
 ...
 p = (unsigned long)tsk + (1 * (1 << 12)) - sizeof(struct pt_regs);
  ...
  unsigned long fn, unsigned long arg)
{
 ...
 if (clone_flags & PF_KTHREAD) {
  ...
  childregs->sstatus = 0x100UL | 0x20UL;

  p->cpu_context.s[0] = fn;
  p->cpu_context.s[1] = arg;

  p->cpu_context.ra = (unsigned long)ret_from_kernel_thread;
 } else {
  *childregs = *task_pt_regs(get_current());
  childregs->a0 = 0;
  if (fn)
   childregs->sp = fn;
...
int do_fork(unsigned long clone_flags, unsigned long fn, unsigned long arg)
{
 ...
 p->pid = pid;
 p->counter = (get_current()->counter + 1) >> 1;
 get_current()->counter >>= 1;
 p->need_resched = 0;
 p->preempt_count = 0;
 p->priority = 2;
 total_forks++;
 g_task[pid] = p;
 do { (p)->next_task = &(init_task_union.task); (p)->prev_task = (init_task_union.task).prev_task; (init_task_union.task).prev_task->next_task = (p); (init_task_union.task).prev_task = (p); } while (0);
 wake_up_process(p);
...
int move_to_user_space(unsigned long pc)
{
 ...
 regs = task_pt_regs(get_current());

 stack = get_free_page();
 if (!stack)
  return -1;

 memset((void *)stack, 0, (1 << 12));

 start_user_thread(regs, pc, stack + (1 << 12));
