...
repeat:
 c = -1000;
 for (tmp = (&rq->rq_head)->next; tmp != (&rq->rq_head); tmp = tmp->next) {
  p = ((struct task_struct *)((char *)(tmp)-(unsigned long)(&((struct task_struct *)0)->run_list)));
  weight = goodness(p);
  if (weight > c) {
   c = weight;
   next = p;
